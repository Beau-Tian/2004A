...
let arr1 = []
 arr.forEach((item,key) =>{
        if( !isNaN(item)){
          return arr1.push(item*key)
           }
    }) 
arr1.sort((a,b) => b-a).filter(item => item>36).join(' ')
"780 170 42"
```
...
```js
let arr3 = arr1.flat(4)
let arr4 = arr2.flat(4)
let arr5 = arr3.concat(arr4)
function fun (arr5) {
  return Array.from(new Set(arr5))
}
let arr6 = fun(arr5)
let arr7 = arr6.sort((a,b) => b-a).join(',')
console.log(arr7)
"50,30,20,15,13,12,10,9,6,5,3,2,1"
```
...
```js
var figures = figure.flat(4)
function unique (figures) {
  return Array.from(new Set(figures))
}
let figure = unique(figures)
let figure  = figure.sort((a,b) => b-a).filter(item => item>4).join(',')
figure  
"9,8,7,6,5"
```
...
```js
let arr1 = Array.from(new Set(objs.map(a =>{return a.a}))).sort((a,b) =>{return b-a})       
 let  arr2 = aa.splice(5,4,10,10,10,10)
...
```js
let arr1 = Array.from(new Set(objs.map(a =>{return a.a}))).sort((a,b) =>{return a-b})       
 let  arr2 = aa.splice(5,4,10,10,10,10)
 arr1.forEach(item =>  console.log(item))
```
...
```js
let arr1 = Array.from(new Set(objs.map(a => { return a.count }).filter(item => item != 30)))
let arr2 = aa.splice(1, 1)
arr1.push(...arr2)
arr1.join(',')
```
