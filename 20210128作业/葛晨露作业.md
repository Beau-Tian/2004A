...
```
var names1 = names.filter(item => item.age >= 40).map(item => item.age/10).sort((a,b)=>b-a);
var names2 = names.filter(item => item.age >= 40).map(item => item.age/10).sort((a,b)=>a-b);
names = names1.concat(names2).join(",")
console.log(names)
```
...
```
figure = Array.from(new Set(figure.flat(5))).filter(item=>item>=5).sort((a,b)=>b-a).join(",").replace(7,'c');
console.log(figure)
```
...
```
objs = Array.from(new Set(objs.map(item => item.a))).sort((a,b)=>b-a).fill(10,5,9).join(",")
console.log(objs)
```
...
```
objs = Array.from(new Set(objs.map(item => item.a))).sort((a,b)=>a-b).fill(10,1,5).join(",")
```
