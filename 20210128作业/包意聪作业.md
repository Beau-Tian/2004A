...
Arr.sort((a,b) => b-a).filter(i => i>36).join(' ')
```
...
//验证过解答:  排序   过滤
let arr = [1, 3, 5, 7, [9, [0, 2, [4, 6, [8, 8, 8]]]]]
arr = arr.flat(100)
let byc = Array.from(new Set(arr)).sort((a, b) => { return b - a }).filter(item => item > 4)
let byc1 = byc.splice(2, 1, "C")
byc.join(' ')
```
...
//验证后解答：
 let byc = Array.from(new Set(objs.map(a =>{return a.a}))).sort((a,b) =>	{return b-a})       
...
var a = 'this is global';
setTimeout(obj.foo, 100);	//?  1      this is  global
```
