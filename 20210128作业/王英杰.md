...
```js
let arr = figure.flat(4)
arr = Array.from(new Set(arr.map((item) => item).sort((a, b) => b - a).filter((item) => item > 4)))
let arr1 = arr.splice(2, 1, "c").join(',')
console.log(arr)
console.log(arr1)
```
...
```js
objs = Array.from(new Set(objs.map(item => item.a).sort((a, b) => b - a))).fill(10,1,5).join(' ')
```
